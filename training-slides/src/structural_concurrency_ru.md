...
Можно рассмотреть два возможных решения: запретить заимствующим тредам перемещаться между любыми тредами или запретить рандеву каналы с незабываемыми типами.
...
Вернёмся к async задачам.
Я подразумеваю что `spawn_scoped` может запустить задачу на другом треде, в отличии от `spawn_local_scoped`.
Другими словами эта функция требует чтобы футура на входе была `Send`.

Если мы сделаем их тред-небезопасными, тогда нам станет тяжело писать вложенные задачи, так как эта тред-небезопасность будет заражать футуру выше и делать её непригодной для параллельной async задачи.
